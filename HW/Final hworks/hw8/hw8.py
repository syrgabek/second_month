...
def select_all_products_whose_price_and_quantity_bigger_than(conn, price, quantity):
    try:
        sql = '''select product_title, price, quantity from products where price <= ? AND quantity >= ?;'''
        c = conn.cursor()
        c.execute(sql, (price, quantity))
...
if conn is not None:
    ...
    print('select_all_products_whose_price_lower_than_100_and_quantity_bigger_than_5')
